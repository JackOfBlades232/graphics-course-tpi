...
void main(void)
{
  ...
  if (lumaRng < max(fxaa311_edge_threshold_min, lumaMax * fxaa311_edge_threshold_max))
  {
  ...
  const float udluma = nluma + sluma;
  const float lrluma = wluma + eluma;
  const float lcluma = nwluma + swluma;
  const float dcluma = swluma + seluma;
  const float rcluma = neluma + seluma;
  const float ucluma = nwluma + neluma;

  // edge
  const float edgeVert =
    fxaa311_high_pass_filter(nluma, ucluma) +
    2.f * fxaa311_high_pass_filter(cluma, lrluma) +
    fxaa311_high_pass_filter(sluma, dcluma);
  const float edgeHoriz =
    fxaa311_high_pass_filter(wluma, lcluma) +
    2.f * fxaa311_high_pass_filter(cluma, udluma) +
    fxaa311_high_pass_filter(eluma, rcluma);
  ...
  [[unroll]]
  for (uint i = 0; i < fxaa311_search_steps; ++i)
  {
    if (!doneP)
    ...
    if (doneP && doneN)
      break;
    if (!doneP)
      offP -= stp * fxaa311_search_qual[i];
    if (!doneN)
      offN += stp * fxaa311_search_qual[i];
  }
  ...
  const float finalOff = edgeEndIsSameAsNei ? sampleOffsetAlongEdge : 0.f;

  // subpixel
  const float lowpassLuma = (1.f / 12.f) * (2.f * (udluma + lrluma) + lcluma + rcluma);
  const float subpixelOffsetBase = clamp(abs(lowpassLuma - cluma) / lumaRng, 0.f, 1.f);
  const float subpixelOffsetAdj = smoothstep(0.f, 1.f, subpixelOffsetBase);
  const float subpixelOffset = subpixelOffsetAdj * subpixelOffsetAdj * fxaa311_subpixel_qual;

#if DEBUG_SUBPIXEL_W
  out_fragColor = vec4(ENCODE_AA_RESULT(vec3(subpixelOffsetAdj + 0.2f)), 1.f);
  return;
#endif

  const float combinedOffset = max(finalOff, subpixelOffset);
  const vec2 finalTc = tc + offBase * constants.mainTargetInverseResolution * combinedOffset;
  const vec3 finalCol = textureLod(ldrImage, finalTc, 0).xyz;
