...
int main(int argc, char** argv)
{
  ...
  {
    tinygltf::Buffer combinedBuffer{};
    combinedBuffer.data.resize(4 * sizeof(Vertex) + 4 * sizeof(Index));
    memset(combinedBuffer.data.data(), 0, 4 * sizeof(Vertex) + 4 * sizeof(Index));

    uint32_t *i = (uint32_t *)((uint8_t *)combinedBuffer.data.data() + 4 * sizeof(Vertex));
    *i++ = 0;
    *i++ = 1;
    *i++ = 2;
    *i++ = 3;
    ...
    vertView.buffer = 0;
    vertView.byteOffset = 0;
    vertView.byteLength = 4 * sizeof(Vertex);
    ...
    indView.name = "indView";
    indView.buffer = 0;
    indView.byteOffset = 4 * sizeof(Vertex);
    indView.byteLength = 4 * sizeof(Index);
    indView.target = TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER;
